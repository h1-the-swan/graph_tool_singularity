...
DESCRIPTION = """Test calculating shortest path distances using graph-tool."""
...
def main(args):
    ...
    n_samples = 1000000
    random_seed = 999
    logger.debug("getting a sample of {} vertices (random seed: {})".format(n_samples, random_seed))
    random_state= np.random.RandomState(random_seed)
    vertices_sample_indexes = random_state.randint(low=0, high=len(name_to_v), size=n_samples)
    vertices_sample = [g.vertex(x) for x in vertices_sample_indexes]
    # vertices_sample is a list of graph-tools Vertex objects
    logger.debug("done getting random sample. took {}".format(format_timespan(timer()-start)))
    ...
    f_calc_times.write("source_index{sep}source_name{sep}calc_time{sep}distance_fname\n".format(sep=sep))
    ...
    for i, source in enumerate(vertices_sample):
        ...
        if os.path.exists(outfname):
            ...
            continue
        logger.debug("calculating shortest distance for vertex: index: {} | name: {}".format(source_index, source_name))
        dist = shortest_distance(g, source=source, target=vertices_sample, directed=directed)
        this_time = timer() - this_start
        with open(outfname, 'w') as outf:
            for x in dist:
                outf.write("{}\n".format(x))
        f_calc_times.write("{source_index}{sep}{source_name}{sep}{calc_time}{sep}{distance_fname}\n".format(sep=sep, source_index=source_index, source_name=source_name, calc_time=this_time, distance_fname=outfname))
